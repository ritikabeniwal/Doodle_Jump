...
static acceleration__axis_data_s consumer_sensor_value[MAX_SIZE];
static TickType_t time_arr[MAX_SIZE];
...
void write_file_using_fatfs_pi(acceleration__axis_data_s *val, TickType_t *time_arr) {
  ...
  if (FR_OK == result) {
    ...
    for (int i = 0; i < MAX_SIZE; i++) {
      sprintf(string + offset, "Time %li, x:%i y:%i z:%i\n", time_arr[i], val[i].x, val[i].y, val[i].z);
...
void consumer_of_accelerator_sensor(void *p) {
  ...
  while (1) {
    ...
    time_arr[i] = xTaskGetTickCount();
    if (i == MAX_SIZE - 1) {
      i = 0;
      write_file_using_fatfs_pi(consumer_sensor_value, time_arr);
    }
    i++;
